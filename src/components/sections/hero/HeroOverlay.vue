 ...
 *
 * Image/video is background, content absolutely positioned based on positionX/Y.
 */
...
const positionX = computed(() => {
  const styles = props.sectionStyles as Record<string, unknown> | undefined
  return (styles?.positionX as string) || 'center'
})

const positionY = computed(() => {
  const styles = props.sectionStyles as Record<string, unknown> | undefined
  return (styles?.positionY as string) || 'middle'
})

const overlayOpacity = computed(() => {
  const styles = props.sectionStyles as Record<string, unknown> | undefined
  return (styles?.overlayOpacity as string) || '50'
})

const overlayBlur = computed(() => {
  const styles = props.sectionStyles as Record<string, unknown> | undefined
  return (styles?.overlayBlur as string) || '0'
})
...
// Overlay does NOT use sectionStyles for background/spacing
// Position is controlled by positionX/Y, overlay by overlayOpacity/overlayBlur
function getOverlayStyle(): Record<string, string> {
  const opacity = parseInt(overlayOpacity.value) / 100
  const blur = parseInt(overlayBlur.value)
