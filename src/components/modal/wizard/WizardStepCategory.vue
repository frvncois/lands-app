    ...
    <div class="grid grid-cols-3 gap-4">
      ...
      >
        <div class="w-12 h-12 rounded-xl bg-primary/10 group-hover:bg-primary/20 flex items-center justify-center transition-colors">
          <Icon :name="getIconName(category.icon)" class="text-primary" :size="24" />
        </div>
        <div class="flex-1">
          <div class="font-semibold text-foreground mb-1">{{ category.name }}</div>
    ...
    <div class="pt-4 border-t border-border">
      ...
      >
        <div class="w-12 h-12 rounded-xl bg-muted group-hover:bg-primary/10 flex items-center justify-center transition-colors">
          <Icon name="+" class="text-muted-foreground group-hover:text-primary" :size="24" />
        </div>
        <div class="text-left">
          <div class="font-semibold text-foreground">Start from scratch</div>
          <div class="text-sm text-muted-foreground">Begin with a blank canvas</div>
        </div>
      </button>
    </div>
  </div>
