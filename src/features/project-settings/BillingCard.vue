...
<template>
  <Card>
    <Card.Header title="Billing" icon="lni-credit-card-1" />
    <Card.Content class="space-y-6">
      <!-- Payment Method -->
      <div class="space-y-3">
        <p class="text-xs font-medium text-muted-foreground uppercase tracking-wider">Payment Method</p>
      ...
      <!-- Invoices -->
      <div class="space-y-3">
        <p class="text-xs font-medium text-muted-foreground uppercase tracking-wider">Past Invoices</p>
          ...
          <table class="w-full">
            ...
            <tbody class="divide-y divide-border">
              <tr v-for="invoice in invoices" :key="invoice.id" class="text-sm">
                <td class="px-3 py-2.5 text-foreground">{{ formatDate(invoice.date) }}</td>
                <td class="px-3 py-2.5 text-foreground">${{ invoice.amount.toFixed(2) }}</td>
                <td class="px-3 py-2.5">
                  <Badge variant="success" size="xs">Paid</Badge>
                </td>
                <td class="px-3 py-2.5 text-right">
                  <Button variant="ghost" size="xs">
                    <Icon name="lni-download-1" class="text-xs" />
                  </Button>
                </td>
              </tr>
            </tbody>
