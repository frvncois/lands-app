...
<template>
  <div class="px-6 py-6 flex flex-col gap-0">
    ...
    <a
      ...
      :href="item.url"
      target="_blank"
      :class="compact ? 'flex items-baseline gap-4 py-2 border-b border-gray-100 last:border-0 group' : 'flex items-baseline gap-4 py-4 border-b border-gray-100 last:border-0 group'"
    >
      <span class="text-xs text-gray-300 w-5 shrink-0 font-mono">{{ String(i + 1).padStart(2, '0') }}</span>
      <div class="flex-1 min-w-0">
        <p :class="compact ? 'text-xs font-medium truncate' : 'text-sm font-medium truncate'" style="color: var(--theme-main)">{{ item.title }}</p>
        <p v-if="item.description && !compact" class="text-xs text-gray-400 truncate mt-0.5">{{ item.description }}</p>
      </div>
